...
export default function RigaFattura({ riga, prodotti, onChange, onRemove }) {
  ...
  const optsCod = useMemo(
    () => prodotti.map((p) => ({ key: p.id, label: p.cod ?? '', detail: [p.descrizione, prezzoLabel(p)].filter(Boolean).join(' — '), raw: p })),
    [prodotti],
  )
  const optsDesc = useMemo(
    () => prodotti.map((p) => ({ key: p.id, label: p.descrizione ?? '', detail: [p.cod, prezzoLabel(p)].filter(Boolean).join(' — '), raw: p })),
    [prodotti],
  )
